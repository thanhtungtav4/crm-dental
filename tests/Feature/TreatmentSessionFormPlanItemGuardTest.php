...
it('binds treatment session form to treatment plan and plan item workflow', function (): void {
    ...
    expect($form)
        ...
        ->toContain("->directory('treatment-sessions/images')")
        ->toContain('normalizeUploadedImagesState')
        ...
        ->not->toContain("KeyValue::make('images')");
});
...
it('normalizes legacy treatment session image payloads into uploaded file lists', function (): void {
    $method = new ReflectionMethod(
        \App\Filament\Resources\TreatmentSessions\Schemas\TreatmentSessionForm::class,
        'normalizeUploadedImagesState',
    );
    $method->setAccessible(true);

    $normalized = $method->invoke(null, [
        'pre_op' => ' treatment-sessions/images/pre-op.jpg ',
        'post_op' => [
            'value' => 'treatment-sessions/images/post-op.png',
        ],
        'empty' => '',
        'invalid' => null,
    ]);
    ...
    expect($normalized)->toBe([
        'treatment-sessions/images/pre-op.jpg',
        'treatment-sessions/images/post-op.png',
    ]);
});
