...
it('renders treatment plan section rows from prepared view state payloads', function (): void {
    ...
    expect($blade)
        ...
        ->and($listSection)
        ->toContain("{{ \$panel['plan_count'] }} hồ sơ")
        ->toContain("@forelse(\$panel['plan_rows'] as \$row)")
        ->toContain("href=\"{{ \$row['plan_url'] }}\"")
        ->toContain("{{ \$row['edit_url'] }}")
        ->toContain("@foreach(\$panel['summary_panels'] as \$summaryPanel)")
        ->and($planModal)
        ->toContain("@if(\$panel['is_visible'])")
        ->toContain("@forelse(\$panel['draft_rows'] as \$row)")
        ->toContain("wire:model=\"draftItems.{{ \$row['index'] }}.diagnosis_ids\"")
        ->and($procedureModal)
        ->toContain("@if(\$panel['is_visible'])")
        ->toContain('wire:click="selectCategory(null)"')
        ->toContain("@foreach(\$panel['categories'] as \$category)")
        ->toContain("@forelse(\$panel['service_rows'] as \$service)")
        ->and($component)
        ->toContain("'viewState' => \$this->sectionViewState(\$sectionData)")
        ->toContain('protected function sectionViewState(array $sectionData): array')
        ...
        ->toContain('protected function categoryRows(Collection $categories): array')
...
it('redirects plan item edit page back to return url after save', function (): void {
    ...
    expect($page)->toContain('protected function getRedirectUrl(): string')
        ...
        ->and($page)->not->toContain('DeleteAction::make()')
        ->and($page)->not->toContain('TreatmentDeletionGuardService')
