...
it('returns transitioned models from direct popup delivery boundaries', function (): void {
    $user = User::factory()->create();
    $user->assignRole('CSKH');

    $announcement = PopupAnnouncement::query()->create([
        'title' => 'Popup direct boundary',
        'message' => 'Direct transition flow',
        'status' => PopupAnnouncement::STATUS_PUBLISHED,
        'target_role_names' => ['CSKH'],
        'target_branch_ids' => [],
        'require_ack' => false,
    ...
    $seenDelivery = $delivery->markSeen();
    $acknowledgedDelivery = $seenDelivery->markAcknowledged();

    expect($seenDelivery)->toBeInstanceOf(PopupAnnouncementDelivery::class)
        ->and($seenDelivery->is($delivery))->toBeTrue()
        ->and($acknowledgedDelivery)->toBeInstanceOf(PopupAnnouncementDelivery::class)
        ->and($acknowledgedDelivery->is($delivery))->toBeTrue()
        ->and($acknowledgedDelivery->status)->toBe(PopupAnnouncementDelivery::STATUS_ACKNOWLEDGED)
        ->and($acknowledgedDelivery->seen_at)->not->toBeNull()
        ->and($acknowledgedDelivery->acknowledged_at)->not->toBeNull();
});
