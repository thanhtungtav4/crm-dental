...
- Recommended direction:
  ...
  - Tien do hien tai:
    ...
    - da tao `ZnsOperationalReadModelService` de gom summary-card counts, dead/retry backlog, va retention candidate rules cho `OpsControlCenterService` cung `CheckObservabilityHealth`, bao gom ca `automation log retention`, thay cho viec page/command tu query status ZNS theo tung chot rieng le
    - da tao `IntegrationOperationalReadModelService` de gom web-lead retention candidates, lead-mail retry/dead backlog, webhook retention, EMR retention, va Google Calendar retention counts cho `OpsControlCenterService` va `ops:check-observability-health`, thay cho viec cockpit/command tu query tung model integration rieng le
- Tests needed:
