...
- Recommended direction:
  ...
  - Tien do hien tai:
    ...
    - da bo sung `PatientOverviewReadModelService` cho `PatientOverviewWidget`, payment summary, va tab counters tren `ViewPatient`, de patient workspace khong con tu giu treatment-plan / appointment / invoice / payment / tab-count aggregate query truc tiep trong widget va page property
