...
- Progress update:
  ...
  - `RRB-010` da bo sung `PatientOverviewReadModelService` de `PatientOverviewWidget`, payment summary, tab counters, material usages, factory orders, va material issue notes trong `ViewPatient` dung chung patient workspace aggregate contract, loai bo them mot lop invoice/payment/appointment/treatment-plan/tab-count/lab-material query khoi widget/page layer.
