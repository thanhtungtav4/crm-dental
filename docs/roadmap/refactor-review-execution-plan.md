...
- Progress update:
  ...
  - `RRB-012` da mo rong them lane financial dashboard convergence bang `FinancialDashboardReadModelService`, de `RevenueOverviewWidget`, `OutstandingBalanceWidget`, `QuickFinancialStatsWidget`, `MonthlyRevenueChartWidget`, `PaymentMethodsChartWidget`, `PaymentStatsWidget`, va ca heading summary + overdue invoice table query cua `OverdueInvoicesWidget` dung chung payment/invoice aggregate contract thay vi moi widget tu giu branch-scoped query logic; `InteractsWithFinancialBranchScope` nay chi con giu visibility gate chung sau khi query helpers duoc rut khoi widget layer.
