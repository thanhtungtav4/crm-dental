...
- Progress update:
  ...
  - `RRB-010` da bo sung `PatientOverviewReadModelService` de `PatientOverviewWidget`, payment summary, va tab counters trong `ViewPatient` dung chung patient workspace aggregate contract, loai bo them mot lop invoice/payment/appointment/treatment-plan/tab-count query khoi widget/page layer.
