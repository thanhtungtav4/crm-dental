...
- Progress update:
  ...
  - `RRB-009` da chuan hoa `ConversationMessage::markSent()` / `markFailed()` / `markIgnored()` tra ve model da transition, de outbound conversation delivery boundaries co return contract nhat quan hon voi cac workflow lanes dang duoc hoi tu.
  - `RRB-009` da chuan hoa `GoogleCalendarSyncEvent::markProcessing()` / `markSynced()` / `markFailure()` / `resetForReplay()` va `EmrSyncEvent::markProcessing()` / `markSynced()` / `markFailure()` / `resetForReplay()` tra ve model da transition, de outbox sync boundaries giua Google Calendar va EMR co return contract nhat quan hon voi cac workflow lanes dang duoc hoi tu.
  - `RRB-009` da chuan hoa `ZnsAutomationEvent::markProcessing()` / `markSent()` / `markFailure()` / `resetForReplay()` / `markSuperseded()` tra ve model da transition, de outbox/event boundaries cua ZNS co return contract nhat quan hon voi cac workflow lanes dang duoc hoi tu.
  - `RRB-009` da chuan hoa `Payment::markReversed()` tra ve model da transition, de lane ledger immutability/reversal metadata co return contract nhat quan hon voi cac workflow boundaries cua FIN.
  - `RRB-009` da chuan hoa `PatientToothCondition::startTreatment()` / `completeTreatment()` tra ve model da transition, de lane benh-an rang-ham-mat co return contract nhat quan hon voi cac workflow boundaries cua TRT.
  - `RRB-009` da chuan hoa `PlanItem::completeVisit()` tra ve model da transition, de visit-progress boundary tren lane dieu tri co return contract nhat quan hon voi cac workflow boundaries cua TRT.
  ...
  - `RRB-009` da chuan hoa `ClinicalResult::markPreliminary()` / `finalize()` / `amend()` tra ve result da transition, de entry points workflow ket qua lam sang co model boundary nhat quan hon.
  - `RRB-009` da bo sung `InsuranceClaim::submit()` / `approve()` / `deny()` / `resubmit()` / `markPaid()` noi ve `InsuranceClaimWorkflowService`, dong thoi chuan hoa cac boundary nay tra ve record/payment da transition de caller layer co contract nhat quan hon.
  ...
  - `RRB-009` da chuan hoa them `ExamSession::transitionTo()` tra ve model da transition, de boundary model entry point cua lane phien kham co contract nhat quan hon voi cac workflow boundary khac.
  ...
  - `RRB-009` da chuan hoa them `InstallmentPlan::syncFinancialState()` va `InstallmentPlanLifecycleService::syncFinancialState()` tra ve plan da sync, de lifecycle facade cua lane tra gop co return contract nhat quan hon voi cac workflow boundaries khac.
  - `RRB-009` da chuan hoa them `Invoice::updatePaidAmount()` / `updatePaymentStatus()` tra ve invoice da sync/mutate, de payment observer, overdue sync, va financial status helpers co return contract nhat quan hon voi cac workflow boundaries cua FIN.
  ...
  - `RRB-009` da chuan hoa `OperationalKpiAlert::markAcknowledged()` / `markResolved()` tra ve model da transition, de KPI alert action boundaries co contract nhat quan hon voi cac workflow lanes dang su dung model-level transition entry points.
  ...
  - `RRB-009` da chuan hoa `WebLeadEmailDelivery::markProcessing()` / `markSent()` / `markFailure()` / `resetForReplay()` tra ve model da transition, de lane outbound web-lead internal mail co model boundary return contract nhat quan hon voi cac workflow lanes khac.
  ...
  - `RRB-009` da chuan hoa them `ZnsCampaignDelivery::claimForProcessing()` / `markSent()` / `markFailure()` tra ve model da transition, de lane campaign delivery cua ZNS co return contract nhat quan hon voi cac delivery/outbox lanes khac.
  ...
  - `RRB-009` da bo sung `ZnsCampaign::schedule()` / `runNow()` noi ve `ZnsCampaignWorkflowService`, de entry points len lich / chay ngay di qua model layer thay vi service call truc tiep.
  ...
  - checkpoint `2026-04-04` da chot them `ViewPatient` o lop tab-shell: page nay da dung `activeWorkspaceTabView()` va tab partials (`basic-info`, `exam-treatment`, `lab-materials`, `payments`, `forms`), de shell chinh khong con giu khoi `@elseif($activeTab === ...)` dai va regression khoa contract partials cua patient workspace.
  - checkpoint `2026-04-04` da chot them lane popup presentation shell: `PopupAnnouncementCenterReadModelService` nay da co `has_announcement` / `aria_live`, va shell popup da tach thanh `popup-announcement-center-shell`, de wrapper view chi con dieu phoi payload presentation-ready.
  - checkpoint `2026-04-04` da mo rong them page-shell convergence sang `ConversationInbox` va `CustomerCare`: `ConversationInbox` da dung `inboxViewState()` cho `queue_panel`, `detail_panel`, `selected_conversation_view`, `lead_modal_view`, va `schema_notice`; `CustomerCare` da dung `careViewState()` cho `overview_panel`, `tabs_panel`, va `active_tab_view`, giam tiep state adapter va markup lap.
  - checkpoint `2026-04-04` da mo rong them page-shell convergence sang `CalendarAppointments`, de page lich hen tong nay tach shell `header / metric cards / filters / reschedule modal` thanh partials va doc presentation payload cho `google_calendar_panel`, `metric_cards`, va `filters_panel` thay vi giu mot Blade dai voi `@php($viewState = ...)`.
  - checkpoint `2026-04-04` da mo rong them page-shell convergence sang `SystemSettings` va `PlaceholderPage`, de hai surface nay dung `pageViewState()` + shell partials thay vi render truc tiep section/bullet state trong Blade.
  ...
  - checkpoint `2026-04-07` da tiep tuc mo rong `BaseReportPage` sang `pageViewState()` + shell partial chung (`report-page-shell`, `report-stat-card`), de lane report co presentation contract cho stat cards va `base-report.blade.php` khong con giu `@php($stats = $this->getStats())` / `@if(!empty($stats))` inline.
  ...
  - checkpoint `2026-04-04` da tiep tuc dua `IntegrationSettings`, `OpsControlCenter`, va `ZaloZns` ve shell/presentation contract manh hon: `provider-health-panel`, `dashboard-summary-card`, `control-plane-section-list`, `control-plane-partial-list`, va `ops-detail-card` da gom them renderer chung cho control-plane pages; regression nay khoa truc tiep `pageViewState()` / `dashboardViewState()` thay vi tiep tuc bam vao adapter state nho le.
