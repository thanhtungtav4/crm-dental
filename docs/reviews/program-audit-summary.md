...
- Checkpoint `2026-04-14` da chuan hoa them `PopupAnnouncementDelivery::markSeen()` / `markAcknowledged()` / `markDismissed()` tra ve model da transition, de direct delivery mutation boundaries co return contract nhat quan hon voi workflow-boundary surfaces cua popup center.
- Checkpoint `2026-04-15` da chuan hoa `ConversationMessage::markSent()` / `markFailed()` / `markIgnored()` tra ve model da transition, de outbound conversation delivery boundaries co return contract nhat quan hon voi cac workflow lanes dang duoc hoi tu.
- Checkpoint `2026-04-15` da chuan hoa `GoogleCalendarSyncEvent::markProcessing()` / `markSynced()` / `markFailure()` / `resetForReplay()` va `EmrSyncEvent::markProcessing()` / `markSynced()` / `markFailure()` / `resetForReplay()` tra ve model da transition, de outbox sync boundaries giua Google Calendar va EMR co return contract nhat quan hon voi cac workflow lanes dang duoc hoi tu.
- Checkpoint `2026-04-15` da chuan hoa `ZnsAutomationEvent::markProcessing()` / `markSent()` / `markFailure()` / `resetForReplay()` / `markSuperseded()` tra ve model da transition, de outbox/event boundaries cua ZNS co return contract nhat quan hon voi cac workflow lanes dang duoc hoi tu.
- Checkpoint `2026-04-15` da chuan hoa `Payment::markReversed()` tra ve model da transition, de lane ledger immutability/reversal metadata co return contract nhat quan hon voi cac workflow boundaries cua FIN.
- Checkpoint `2026-04-15` da chuan hoa `PatientToothCondition::startTreatment()` / `completeTreatment()` tra ve model da transition, de lane benh-an rang-ham-mat co return contract nhat quan hon voi cac workflow boundaries cua TRT.
- Checkpoint `2026-04-15` da chuan hoa `PlanItem::completeVisit()` tra ve model da transition, de visit-progress boundary tren lane dieu tri co return contract nhat quan hon voi cac workflow boundaries cua TRT.
- Checkpoint `2026-04-15` da chuan hoa `ZnsCampaignDelivery::claimForProcessing()` / `markSent()` / `markFailure()` tra ve model da transition, de lane campaign delivery cua ZNS co return contract nhat quan hon voi cac delivery/outbox workflow boundaries dang duoc hoi tu.
- Checkpoint `2026-04-15` da chuan hoa `ExamSession::transitionTo()` tra ve model da transition, de model entry point cua lane phien kham co return contract nhat quan hon voi cac workflow boundaries cua CLIN / TRT.
- Checkpoint `2026-04-15` da chuan hoa `InstallmentPlan::syncFinancialState()` va `InstallmentPlanLifecycleService::syncFinancialState()` tra ve plan da sync, de lifecycle facade cua lane tra gop co return contract nhat quan hon voi cac workflow boundaries cua FIN.
- Checkpoint `2026-04-15` da chuan hoa `Invoice::updatePaidAmount()` / `updatePaymentStatus()` tra ve invoice da sync/mutate, de cac financial status boundaries cua FIN co return contract nhat quan hon voi workflow/lifecycle lanes lien quan.
- Checkpoint `2026-04-15` da chuan hoa `PlanItem::updateProgress()` / `TreatmentPlan::updateProgress()` tra ve model da mutate, de parent-child progress boundaries cua lane dieu tri co return contract nhat quan hon voi cac workflow boundaries cua TRT.
- Checkpoint `2026-04-15` da chuan hoa `ClinicalNote::syncExamSessionSnapshot()` tra ve clinical note da sync, de boundary dong bo phien kham cua lane clinical note co return contract nhat quan hon voi cac workflow boundaries cua CLIN.
- Checkpoint `2026-04-15` da mo rong `PatientExamSessionReadModelService` them `latestSession()` / `findSession()`, de `PatientExamForm` bo query truc tiep `examSessions()->with('clinicalNote')` khoi `mount()`, `setActiveSession()`, `startEditingSession()`, va lane chon session tiep theo sau delete, tiep tuc lam mong component layer cua `Khám bệnh`.
...
- Checkpoint `2026-04-14` da chuan hoa `ClinicalResult::markPreliminary()` / `finalize()` / `amend()` tra ve result da transition, de entry points workflow ket qua lam sang co model boundary nhat quan hon.
- Checkpoint `2026-04-14` da bo sung `InsuranceClaim::submit()` / `approve()` / `deny()` / `resubmit()` / `markPaid()` canonical noi ve `InsuranceClaimWorkflowService`, dong thoi chuan hoa cac boundary nay tra ve record/payment da transition de caller layer co contract nhat quan hon.
...
- Checkpoint `2026-04-14` da chuan hoa `OperationalKpiAlert::markAcknowledged()` / `markResolved()` tra ve model da transition, de KPI alert action boundaries co contract nhat quan hon voi cac workflow lanes dang su dung model-level transition entry points.
- Checkpoint `2026-04-14` da chuan hoa `WebLeadEmailDelivery::markProcessing()` / `markSent()` / `markFailure()` / `resetForReplay()` tra ve model da transition, de lane outbound web-lead internal mail co model boundary return contract nhat quan hon voi cac workflow lanes khac.
...
- Checkpoint `2026-04-16` da dong RRB-011: 3 test files moi cover end-to-end reconciliation (`WalletLedgerReconciliationTest` — 8 tests: deposit/spend/refund/idempotency/adjustment audit/negative guard), concurrency (`PaymentReversalConcurrencyTest` — 5 tests: concurrent reversal single record, idempotency, audit log, wallet CREDIT, amount guard), va model-level immutability (`ImmutableLedgerBoundaryContractTest` — 6 tests: `WalletLedgerEntry` update+delete blocked, firstOrCreate idempotency, `InventoryTransaction` update+delete blocked, reversal CREDIT direction); tong 38/38 pass (148 assertions) cho full RRB-011 suite; `RRB-011-plan.md` da duoc tao va `status: Completed`.
- Checkpoint `2026-04-16` da dong RRB-012: 1 test file moi `ReportDateRangeBoundaryContractTest` (9 tests, 19 assertions) cover cashflow date-range boundary, appointment boundary, patient boundary, material branch isolation, null branchIds admin view; tong 78/78 pass (766 assertions) cho full RRB-012 suite bao gom 9 pre-existing report test files; `RRB-012-plan.md` da duoc tao va `status: Completed`.
- Checkpoint `2026-04-16` da dong RRB-013: 1 test file moi `IntegrationProviderCatalogCrossReferenceTest` (11 tests, 189 assertions) cover 7-provider catalog completeness, structural key contract, counts sum, unknown-key exception, 5-provider readiness surface, notification keys, dead-letter backlog readers cho EMR va Google Calendar; tong 78/78 pass voi 9 pre-existing integration test files; `RRB-013-plan.md` da duoc tao va `status: Completed`.
- Checkpoint `2026-04-16` da dong RRB-010: tat ca 26 read-model/timeline service modules da co contract tests Pest day du (bao gom `CustomerCareSlaReadModelService`, `ConversationInboxReadModelService`, `OpsAutomationCatalog`, `OpsReleaseGateCatalog`); 45 tests moi (363 assertions) pass tren 3 test files moi, tong hop voi timeline suite 15/15; `RRB-010-plan.md` da duoc tao va `status: Completed`.
- Checkpoint `2026-04-16` da dong RRB-009: structured `AuditLog::record()` voi `trigger`, `status_from`, `status_to`, `reason`, `actor_id` da duoc chuyen vao tat ca 7 workflow service modules (`WebLeadInternalEmailNotificationService`, `AppointmentSchedulingService`, `PaymentReversalService`, `PopupAnnouncementDeliveryWorkflowService`, `ZnsCampaignDelivery`, `PlanItemWorkflowService`, `ZnsCampaignWorkflowService`) kem contract tests Pest (72/72 pass, 355 assertions tren 10 test files); `RRB-009-plan.md` da duoc cap nhat `status: Completed`.
- Checkpoint `2026-04-16` da dong RRB-014: 1 test file moi `CrossModuleTrtInvFinChainSmokeTest` (10 tests, 28 assertions) cover end-to-end TRT→INV→FIN chain: usage→stock decrease, reversal→stock restored, InventoryTransaction immutability, double-reversal idempotency, audit log, invoice→payment, wallet DEBIT, payment reversal→wallet CREDIT, branch/patient attribution consistency; `RRB-014-plan.md` da duoc tao va `status: Completed`.
- Checkpoint `2026-04-16` da dong RRB-015: 1 test file moi `KpiSnapshotSlaAndOpsReleaseGateContractTest` (12 tests, 43 assertions) cover `resolveSlaStatus()` on_time/late/stale/missing classification, `checkScope()` placeholder creation va dry-run, `ZnsOperationalReadModelService::automationDeadCount()` scoping, `OpsReleaseGateCatalog::steps()` structure va `requiredProductionCommands()` completeness; `RRB-015-plan.md` da duoc tao va `status: Completed`.
- Checkpoint `2026-04-16` da dong RRB-016: 1 test file moi `ProductionDatasetSmokeAndMpiChainTest` (7 tests, 23 assertions) cover `invoiceBalanceSummary()` cross-branch isolation + date range filter + empty guard, MPI merge reparents TreatmentPlan+Invoice sang canonical patient, MPI rollback khoi phuc references ve merged patient; `RRB-016-plan.md` da duoc tao va `status: Completed`.
...
1. `RRB-009` va `RRB-010` da duoc dong. Tiep theo uu tien `RRB-011` (immutable ledger) va `RRB-012`/`RRB-013` song song.
