...
- Checkpoint `2026-04-14` da bo sung `ClinicalOrder::markInProgress()` boundary co `reason` / `actor` / `trigger` noi ve `ClinicalOrderWorkflowService`, de entry point cap nhat in-progress di qua model layer thay vi service call truc tiep.
